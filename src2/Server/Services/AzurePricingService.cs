...
public class AzurePricingService
{
    ...
    public async Task<List<SearchRate>> GetRatesAsync(string serviceName, string location, string currencyCode)
    {
        List<SearchRate> rates = new();
        AzurePrices? results;
        bool loop;

        string url = configuration.GetSection("AzureServices").GetSection("SearchUrl").Value.ToString();
        url = url.Replace("{ServiceName}", serviceName).Replace("{Location}", location).Replace("{CurrencyCode}", currencyCode);
        ...
        do
        {
            ...
            foreach (var item in results.Items)
            {
                if (item is not null && !string.IsNullOrEmpty(item.ServiceId))
                {
                    rates.Add(item);
                }
            }
